...
// Fun with encoding (+simple muxing) with classes
...
void printError(int err) {
    using namespace std;
    string buf(1024, ' ');
    av_strerror(err, (char *)buf.c_str(), 1024);
    cout << buf << endl;
}
//======================================================================================================================
/// A trivial muxer for 1 video stream, write to a file
class MuxerOneVideoStream {
public: //=========== Methods
    explicit MuxerOneVideoStream(const std::string &fileName, const AVCodecContext *pCodecCtx) {
        using namespace std;
        CV_Assert(0 <= avformat_alloc_output_context2(&pFmtCtx, nullptr, nullptr, fileName.c_str()));
        stream = avformat_new_stream(pFmtCtx, nullptr);
        CV_Assert(stream);
        CV_Assert(0 <= avcodec_parameters_from_context(stream->codecpar, pCodecCtx));
        stream->id = 0;
        stream->time_base = AVRational{1, 30};
        avio_open(&pFmtCtx->pb, fileName.c_str(), AVIO_FLAG_WRITE);
//        int ret = avformat_write_header(pFmtCtx, nullptr);
//        cout << "ret=" << ret << ", " << AVERROR_INVALIDDATA <<  endl;
//        printError(ret);
        CV_Assert(0 <= avformat_write_header(pFmtCtx, nullptr));
    }
    ...
    void write(AVPacket *pPacket) {
        av_write_frame(pFmtCtx, pPacket);
    }
